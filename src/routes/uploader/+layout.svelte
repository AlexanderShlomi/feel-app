<script>
    ...
    /** When true, collection + uploaders stay mounted but hidden (display:none) so decoded images are retained. */
    $: isMagnetEditorRoute = $page.url.pathname.includes('/uploader/edit/');

    afterNavigate(({ from, to }) => {
        try {
            const fromPath = from?.url?.pathname || '';
            const toPath = to?.url?.pathname || '';
            if (toPath === '/uploader' && fromPath.includes('/uploader/edit/')) {
...
<div
     class="uploader-workspace"
     class:uploader-workspace--hidden={isMagnetEditorRoute}
     aria-hidden={isMagnetEditorRoute ? 'true' : undefined}
>
...
<div class="uploader-child-slot" class:uploader-child-slot--active={isMagnetEditorRoute}>
    <slot />
</div>
</div>

<style>
    /* Persisted workspace: hide during single-magnet edit without unmounting (keeps decoded images). */
    .uploader-workspace--hidden {
    ...
    .magnet-wrapper {
        ...
        contain-intrinsic-size: 216px 216px;
    }
    /* בפסיפס – תאים תמיד ריבועיים (גם בתמונות אופקיות) */
    .canvas-container.split-center .magnet-wrapper {
        aspect-ratio: 1 / 1;

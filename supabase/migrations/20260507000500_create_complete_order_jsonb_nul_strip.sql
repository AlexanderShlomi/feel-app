-- Strip U+0000 from every string inside jsonb before casts (->>), ::text, or
-- TEXT inserts. Client-side cleaning is not enough when NUL hides in nested
-- configuration or when older bundles skip sanitization.
...
begin
  ...
  if t = 'string' then
    return to_jsonb(replace(p_input #>> '{}', chr(0), ''));
